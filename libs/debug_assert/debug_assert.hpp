...
#define DEBUG_ASSERT_CUR_SOURCE_LOCATION                                       \
  debug_assert::source_location { __FILE__, static_cast<unsigned>(__LINE__) }
...
template <bool Value, typename T = void> struct enable_if;
...
template <class Handler, typename = void> struct allows_exception {
  static const bool value = false;
};

template <class Handler>
struct allows_exception<
    Handler, typename enable_if<Handler::throwing_exception_is_allowed>::type> {
...
constexpr auto
do_assert(const Expr &expr, const source_location &loc, const char *expression,
          Handler, level<Level>,
  ...
  return expr() ? regular_void()
                : debug_assertion_failed<Handler>(
                      loc, expression, detail::forward<Args>(args)...);
}

template <class Expr, class Handler, unsigned Level, typename... Args>
DEBUG_ASSERT_FORCE_INLINE constexpr auto
do_assert(const Expr &, const source_location &, const char *, Handler,
...
constexpr bool always_false() noexcept { return false; }
} // namespace detail
} // namespace debug_assert
